...
int main() {
	printf("模拟进程调度（HRRN算法） 作者黄家伟，HNU计科院OS课程设计\n\n");
	...
	for (int i = 0; i < MAX_TIME; i++) {
		...
		if(head_ready != NULL){
			...
			for(;Cal->next != NULL;){
				Cal->priority = (Cal->alltime + i + 0.0)/Cal->alltime;
				printf("进程%d",Cal->id);
				printf("优先级为：%f  ",Cal->priority);
				if(Cal->next != NULL){
					Cal = Cal->next;
				}
			}
			...
			printf("进程%d",Cal->id);
			printf("优先级为：%f  ",Cal->priority);
			//注释：本轮才加入到就绪队列中的在本时间片最后才开始计算优先权";
			sort_ready_queue();
		}
	}
	return 0;

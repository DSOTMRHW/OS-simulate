...
typedef struct PCB {
	int id;  // 进程标识数 ID
	double priority;  // 进程优先数 PRIORITY
	double alltime;  // 进程运行所需 CPU总时间片数 ALLTIME
...
// 创建进程控制块
PCB *create_pcb(int id, double priority, double alltime, int startblock, int blocktime) {
...
void sort_ready_queue() {
	...
	while (p != NULL) { // 循环遍历就绪队列
		...
		while (q != NULL) { // 再次循环遍历就绪队列
			if (p->priority < q->priority) { // 如果p的总时间大于q的总时间
				// 交换p和q的信息
				int temp_id = p->id;
				double temp_priority = p->priority;
				double temp_alltime = p->alltime;
...
int main() {
	printf("模拟进程调度（NP-DPS算法） 作者黄家伟，HNU计科院OS课程设计\n\n");
	
	// 创建进程控制块并插入就绪队列
	PCB *pcb1 = create_pcb(1, 1, 6.0, 2, 3); //create_pcb(int id, int priority, int alltime, int startblock, int blocktime)
	insert_ready_queue(pcb1);
	PCB *pcb2 = create_pcb(2, 2, 4.0, 3, 1); //create_pcb(int id, int priority, int alltime, int startblock, int blocktime)
	insert_ready_queue(pcb2);
	PCB *pcb3 = create_pcb(3, 3, 8.0, 4, 1); //create_pcb(int id, int priority, int alltime, int startblock, int blocktime)
	insert_ready_queue(pcb3);
	PCB *pcb4 = create_pcb(4, 4, 5.0, 2, 3); //create_pcb(int id, int priority, int alltime, int startblock, int blocktime)
	insert_ready_queue(pcb4);
	PCB *pcb5 = create_pcb(5, 5, 7.0, 1, 2); //create_pcb(int id, int priority, int alltime, int startblock, int blocktime)
	...
	for (int i = 0; i < MAX_TIME; i++) {
		...
		if (running != NULL) {
			...
			if((running->priority-3) <= 0){
				running->priority = 0;
			}
			else
				running->priority = running->priority - 3;
			sort_ready_queue();
		...
		if(head_ready != NULL){
			if(Cal->next == NULL || Cal != head_ready){
				Cal = head_ready;
			}
			for(;Cal->next != NULL;){
				Cal->priority = Cal->priority + 3;
				printf("进程%d优先级为：%f  ",Cal->id,Cal->priority);
				if(Cal->next != NULL){
					Cal = Cal->next;
				}
			}
